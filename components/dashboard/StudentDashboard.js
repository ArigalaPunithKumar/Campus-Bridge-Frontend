...
const CodingArena = ({ selectedLanguage, setSelectedLanguage, code, setCode, isRunning, setIsRunning, compilerInput, setCompilerInput, compilerOutput, setCompilerOutput }) => {
    ...
    const handleRunCode = async () => {
        ...
        try {
            const res = await axios.post("https://campus-bridge-backend.onrender.com/api/compiler/execute", {
...
const AssignmentsView = ({ assignments, setAssignments, user, code }) => {
    ...
    const handleUpload = async () => {
        ...
        try {
            ...
            await axios.post("https://campus-bridge-backend.onrender.com/api/student/submit", formData, {
...
const SettingsView = ({ settings, setSettings, user }) => {
    ...
    const toggleSetting = async (key) => {
        ...
        // Save to Backend
        try {
            await axios.put(`https://campus-bridge-backend.onrender.com/api/student/settings/${user.id}`, newSettings);
        } catch (err) {
...
const ChatAssistant = ({ showAssistant, setShowAssistant, code, language }) => {
    ...
    const handleSend = async () => {
        ...
        setIsLoading(true);
        try {
            const res = await axios.post("https://campus-bridge-backend.onrender.com/api/compiler/analyze", {
...
const StudentDashboard = () => {
    ...
    const fetchStudentData = async (studentId) => {
        try {
            const res = await axios.get(`https://campus-bridge-backend.onrender.com/api/student/dashboard/${studentId}`);
    ...
    const fetchNotifications = async (studentId) => {
        try {
            const res = await axios.get(`https://campus-bridge-backend.onrender.com/api/notifications/${studentId}`);
    ...
    const fetchSchedule = async (studentId) => {
        try {
            const res = await axios.get(`https://campus-bridge-backend.onrender.com/api/student/schedule/${studentId}`);
    ...
    const markNotificationRead = async (id) => {
        try {
            await axios.put(`https://campus-bridge-backend.onrender.com/api/notifications/read/${id}`);
    ...
    const clearAllNotifications = async () => {
        try {
            await axios.delete(`https://campus-bridge-backend.onrender.com/api/notifications/clear/${user.id}`);
            setNotifications([]);
            setUnreadCount(0);
        } catch (err) {
